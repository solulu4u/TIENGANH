...
const MultiplayerGame: React.FC = () => {
  ...
  const [realTimeScores, setRealTimeScores] = useState<{[playerId: string]: number}>({});
    ...
    if (foundRoom) {
      ...
      setCurrentLesson({
        ...
        challenges: [
          {
            id: '1',
            content: 'Coffee is one of the most popular beverages in the world today.',
          ...
          {
            id: '2', 
            content: 'The story of coffee begins in ancient Ethiopia, where legend says a goat herder discovered the energizing effects of coffee beans.',
  ...
  const resetTimer = () => {
    ...
    setGameState(prev => ({ ...prev, timeRemaining: room?.settings.timeLimit || 60 }));
    startTimer();
  };
  ...
  const submitAnswer = () => {
    ...
    const comparison = compareWordsDetailed(userAnswer, currentChallenge.content);
    const isCorrect = comparison.every(word => word.status === 'correct');
    const score = isCorrect ? 10 : Math.max(0, 10 - comparison.filter(w => w.status !== 'correct').length);
    ...
    sendMessage({
      ...
      data: {
        ...
        score,
        isCorrect,
        timeSpent: (room?.settings.timeLimit || 60) - gameState.timeRemaining
      }
    });
  };

  const getPlayerRank = (playerId: string) => {
    const scores = Object.entries(realTimeScores).sort(([,a], [,b]) => b - a);
  ...
  if (!room || !currentLesson) {
    return (
      <div className="p-6 text-center">
        <h2 className="text-xl font-semibold text-slate-800">Loading game...</h2>
      </div>
    );
  }
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
          <h3 className="text-2xl font-bold text-slate-800 mb-6 text-center">Leaderboard</h3>
          <div className="space-y-4">
            {sampleGameResults.finalScores.map((result, index) => (
              <div key={result.playerId} className={`flex items-center justify-between p-4 rounded-lg ${
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <div className="font-semibold text-slate-800">{result.playerName}</div>
                    <div className="text-sm text-slate-600">
                      {result.correctAnswers}/{result.totalAnswers} correct • Avg: {result.averageTime}s
                    </div>
                  </div>
                </div>
      ...
      <div className="bg-white shadow-sm border-b border-slate-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-600">Multiplayer Dictation Battle</p>
          </div>
          <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-2">
              <Clock className="w-5 h-5 text-slate-600" />
              <span className={`font-bold text-lg ${gameState.timeRemaining <= 10 ? 'text-red-600' : 'text-slate-800'}`}>
                ...
                {room.players
                  ...
                  <div key={player.id} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg">
                    ...
                    <div className="flex items-center space-x-1">
                      <span className="font-bold text-slate-800">{realTimeScores[player.id] || 0}</span>
                      {hasSubmitted && player.id === user?.id && (
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className={`w-full h-32 p-4 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none text-lg ${
                      hasSubmitted ? 'bg-gray-50 cursor-not-allowed' : 'border-slate-300'
                    }`}
                  />
                </div>
              ...
              <div className="mt-6 pt-6 border-t border-slate-200">
                <h4 className="font-medium text-slate-800 mb-3">Activity Feed</h4>
                <div className="space-y-2 max-h-32 overflow-y-auto">
                  <div className="flex items-center space-x-2 text-sm text-slate-600">
                    <Zap className="w-4 h-4 text-blue-500" />
                    <span>Sarah Chen submitted their answer</span>
                    <span className="text-xs text-slate-400">2s ago</span>
                  </div>
                  <div className="flex items-center space-x-2 text-sm text-slate-600">
                    <Zap className="w-4 h-4 text-green-500" />
                    <span>Michael Rodriguez got it correct!</span>
