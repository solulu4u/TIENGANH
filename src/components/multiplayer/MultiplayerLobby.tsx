...
const MultiplayerLobby: React.FC = () => {
  ...
  const { isConnected, lastMessage, sendMessage, error } = useWebSocket('ws://localhost:8080/multiplayer');
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-8 text-white">
          <div className="flex items-center space-x-4 mb-6">
            ...
            <div>
              ...
              <p className="text-blue-100">Start a new multiplayer session</p>
            </div>
          </div>
          <button
      ...
      {/* Active Rooms */}
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-slate-800 mb-6">Active Rooms</h2>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {rooms.map((room) => (
            <div key={room.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-lg transition-all duration-200">
              <div className="p-6">
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                  ...
                  <div className="text-center p-3 bg-slate-50 rounded-lg">
                    ...
                    <div className="text-sm font-medium text-slate-800">
                      {room.settings.lessonSelection === 'random' ? 'Random' : 'Host Choice'}
                    </div>
                ...
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  {room.players.length > 4 && (
                    <div className="w-8 h-8 bg-slate-200 rounded-full flex items-center justify-center">
                      <span className="text-slate-600 text-xs font-bold">+{room.players.length - 4}</span>
                    </div>
                  )}
                </div>
                ...
                >
                  ...
                  <span>
                    {room.status === 'waiting' ? 'Join Room' : 
                     room.status === 'playing' ? 'In Progress' : 'Room Full'}
                  </span>
                </button>
              </div>
            </div>
      ...
      {showCreateRoom && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            <h3 className="text-2xl font-bold text-slate-800 mb-6">Create New Room</h3>
      ...
      {showJoinRoom && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            <h3 className="text-2xl font-bold text-slate-800 mb-6">Join Room</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  Room Code
